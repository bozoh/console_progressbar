...
class TestConsoleProgressBar(TestCase):
    ...
    def test_it_has_custom_prefix(self):
        pb = ProgressBar(self.total, prefix="Teste")
        self.assertTrue("Teste" in pb.generate_pbar(50))
        
    
    def test_it_has_custom_suffix(self):
        pb = ProgressBar(self.total, suffix="Teste")
        self.assertTrue("Teste" in pb.generate_pbar(50))
    ...
    def test_it_has_custom_decimals_palces(self):
        pb = ProgressBar(98, decimals = 1)
        # 100*42/98 = 42,85714285714285714286
        self.assertRegexpMatches(pb.generate_pbar(42),'\.\d{1}\%')
        self.assertNotRegexpMatches(pb.generate_pbar(42),'\.\d{2}\%')
        self.assertNotRegexpMatches(pb.generate_pbar(42),'\.\d{3}\%')
        pb = ProgressBar(98, decimals = 2)
        self.assertNotRegexpMatches(pb.generate_pbar(42),'\.\d{1}\%')
        self.assertRegexpMatches(pb.generate_pbar(42),'\.\d{2}\%')
        self.assertNotRegexpMatches(pb.generate_pbar(42),'\.\d{3}\%')
        pb = ProgressBar(98, decimals = 3)
        self.assertNotRegexpMatches(pb.generate_pbar(42),'\.\d{1}\%')
        self.assertNotRegexpMatches(pb.generate_pbar(42),'\.\d{2}\%')
        self.assertRegexpMatches(pb.generate_pbar(42),'\.\d{3}\%')
